...
::: {.callout-tip}
**Arbejder du på DARTER?** Bekræftede stier og modificeringsdatoer for projekt 708421 findes på [Registersti og datastores →](darter/registersti-datastores.qmd)
:::
...
::: {.callout-note}
**Join-nøgler** er den kolonne to tabeller har til fælles — det R bruger til at matche rækker, når du sætter dem sammen med `left_join()` eller `inner_join()`.
Eks.: `left_join(lpr_adm, lpr_diag, by = "recnum")` finder rækker i begge tabeller med samme `recnum`.
