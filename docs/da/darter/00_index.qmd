...
::: {.callout-tip}
**Alternativ: brug `compute()` til DuckDB-forbindelse**
`load_database()` returnerer en Arrow-forbindelse der kun understøtter et subset af dplyr-funktioner.
Har du brug for DuckDB-specifik funktionalitet eller oplever du langsom ydeevne, kan du pipe til `compute()`:
...
Reducer altid data med `filter()`/`select()` inden `compute()`. Se [osdc-dokumentationen](https://steno-aarhus.github.io/osdc/) for DuckDB-konfiguration og memory limits.
:::
...
- [Fase 9 — Hospitalskontakter (LPR)](../09_hospitalskontakter-lpr.qmd) — forklaringen bag to-fase strategien, LPR2/LPR3 og D-præfikset
