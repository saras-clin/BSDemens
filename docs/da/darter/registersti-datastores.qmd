...
**DODSAARS og DOD:** `dodsaars` er i cleaned-data men dækker kun ~1970–2001 (pr. juni 2026). For post-2001 dødsfald kræves DOD-udtræk fra rå SAS — guide under udvikling.
...
# Psykiatrisk LPR2 — kræver rename:
psyk_adm <- load_database("t_psyk_adm") %>%
  rename_with(tolower) %>%
  rename(pnr = v_cpr, recnum = k_recnum)

# DBSO — parquet-external:
dbso <- load_database("dbso") %>% rename_with(tolower)

# OSDC — RDS-fil:
dm_pop <- readRDS(path_dm_pop) %>% rename(pnr = PNR)

# LPR3 procedurer — join via dw_ek_forloeb (IKKE dw_ek_kontakt):
...
- [Registerreference](../register_reference.qmd) — fulde bekræftede kolonnenavne
- [DARTER — faldgruber](darter-faldgruber.qmd) — projekt-specifikke problemer
