---
title: "Joins og pivots — saml dine udtræk"
subtitle: "Forme, kombinere og klargøre dit analysedatasæt"
date: last-modified
---

Du har nu lavet en række udtræk og gemt dem som RDS-filer — ét udtræk per register. Inden du kan analysere, skal du:
...
Hvert udtræk fra Fase 6, 9, 11 m.fl. bør gemmes som sin egen RDS-fil — ét register, ét emne. Eksempel på hvad du typisk har:
...
| `extract_dodsaars.rds` | Dødsdato | `pnr`, `death_date` (NA = lever) |
...
I stedet bruger du mønstret fra [Fase 9](09_hospitalskontakter-lpr.qmd): udtræk første dato for én specifik diagnosegruppe, gem som `extract_demens.rds`, og join den som én kolonne med `left_join()`. Én udfaldsvariabel = én kolonne = én RDS-fil.
:::
...
bef_en_per_person <- bef %>%
  ...
  ungroup()                  # frigiv gruppering — vigtigt: glem ikke ungroup()
```
...
`bind_rows()` — når du har **to versioner af det samme register** (LPR2 + LPR3, eksponerede + komparatorer) og vil slå dem sammen til ét.
...
Funktionerne brugt her er forklaret i [Fase 14 — Funktioner: oversigt](14a_guide_til_funktioner.qmd) (`group_by`, `slice`, `transmute`, `left_join`) og [Fase 12](13a_pakker-oversigt.qmd) (`riskSetMatch`).
...
→ [Fase 12 — Socioøkonomiske variable](12_socioekonomiske-variable.qmd)

## Se også

- [Fase 14 — Funktioner: oversigt](14a_guide_til_funktioner.qmd) — alle funktioner du bruger til at transformere data
