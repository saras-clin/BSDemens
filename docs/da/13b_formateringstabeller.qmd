...
Kender du ikke joins endnu, læs [Fase 10 — Dataformater og manipulation](10_data-manipulation.qmd) først.
:::
...
::: {.callout-warning}
**Ikke verificeret endnu.** Basestien ovenfor og de konkrete filnavne og kolonnenavne længere nede på siden (fx `c_kom_v4_t.sas7bdat`, `KOM_V4_T`) er **illustrative eksempler** — de er endnu ikke bekræftet mod DST-serveren. Brug stjerne-guiden på skrivebordet og `names()`/`head()` til at finde de faktiske navne, inden du stoler på dem.
:::

Undermapper er organiseret efter emne:
```
...
bef_med_kommune <- bef_data %>%
  ...
  left_join(kom_tabel, by = c("bopkom" = "START"))
```
...
```r
names(kom_tabel)   # se hvad kolonner hedder
...
- [Socioøkonomiske variable](11_socioekonomiske-variable.qmd) — SEPLINE-tilgang med kodeeksempler for AKM, FAIK og UDDA
