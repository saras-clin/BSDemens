...
subtitle: "Bekræftede kolonnenavne for alle 14 registre — projekt 708421"
date: last-modified
format:
  html:
    toc: true
...
Se [Faldgruber](dst_faldgruber.qmd) for særlige quirks ved hvert register.
...
::: {.callout-warning}
**Stavemåde:** kolonnen hedder `senere_afkraeftet` (dansk) — ikke `senare_afkraeftet` (svensk).
Forkert stavemåde giver ingen fejl, men ingen diagnoser ekskluderes.
:::
...
## 5. LMDB — Lægemiddelstatistikregistret
...
Én post per person per år, uddannelsesniveauet ændres.
...
1. **Datokolonner er character-strenge** — kræver `as.Date()`. Undtagelse: `foerstefedmeopr_dato`.
...
| **`datoper_prim`** | character | **Operationsdato** (primær BS) — konverter med `as.Date()` |
...
| `datopre` | character | Præoperativt besøg — konverter med `as.Date()` |
| `datofol` | character | Opfølgningsbesøg — konverter med `as.Date()` |
