...
| En operationsprocedure / SKS-kode (2018 og frem) | procedurer_kirurgi | `procedurekode`, `dw_ek_kontakt` |
...
`lpr_sksopr` joines til `lpr_adm` på `recnum`; `procedurer_kirurgi` joines til `lpr_a_kontakt` på `dw_ek_kontakt` (efter `rename_with(tolower)`).
:::
...
| DODSAARS | `"dodsaars"` | `pnr` | Alle år | **`d_dodsdto`** (dødsdato til censurering) |
...
| LPR3 SKS-procedurer | `"procedurer_kirurgi"` | `DW_EK_KONTAKT` | 2018+ | `procedurekode`, `dato_start` |
...
### procedurer_kirurgi — LPR3 SKS-procedurer (2018 og frem)
...
| `proceduretype` | character | Proceduretype-kode |
...
::: {.callout-warning}
Kolonnenavnene i `procedurer_kirurgi` er blandet store/små bogstaver i rådata. Kald `rename_with(tolower)` umiddelbart efter load — ellers fejler joins.
:::
